...
tf.app.flags.DEFINE_integer('num_reducers', 1, 'How parameter servers.')
tf.app.flags.DEFINE_integer('iters', 100, 'How solver iterations.')
tf.app.flags.DEFINE_integer('num_tests', 6, 'Num of tests.')
tf.app.flags.DEFINE_integer('checkpoint_stereducer', -1, 'Stereducer between checkpoint.')
...
def build_parameters(N, num_worker):
    ...
    with tf.variable_scope('reducer-0'):
        with tf.device('/job:reducer/task:0/cpu:0'):
            ...
            reducer_delta_old = tf.get_variable(name='delta_old', initializer=np.load('data/'+str(N)+'/delta.npy').astype(np.float64)[0][0], dtype=tf.float64, use_resource=True)
    ...
    with tf.variable_scope('task-'+str(task_index)):
        ...
        with tf.device('/job:worker/task:%d/gpu:0' % task_index):
            ...
            with tf.control_dependencies([compute_partial_alpha]):
                ...
                update_alpha = tf.assign(alpha, alpha_queue.dequeue())
                ...
                with tf.control_dependencies([update_alpha]):
                    ...
                    with tf.control_dependencies([compute_r_local, compute_x_local]):
                        ...
                        compute_partial_delta = partial_delta_queue.enqueue(tf.reduce_sum(tf.matmul(r_local, r_local, transpose_a=True)))
                        ...
                        with tf.control_dependencies([compute_partial_delta]):
                            ...
                            compute_p_local = p_queue.enqueue((task_index, x_local, tf.add(tf.scalar_mul(beta, p_local), r_local)))
...
def create_cluster(job_name, task_index):
    ...
    cluster = tf.train.ClusterSpec({ "reducer": tf_hostlist[0:num_reducers], "worker": tf_hostlist[num_reducers:] })
...
def run_reducer(task_index, chief_only_hooks, hooks):
    ...
    barrier = build_barrier(int(os.environ['SLURM_NTASKS']))
    ...
    for _ in range(FLAGS.num_tests):
                            ...
                            #save_checkpoint_stereducer=FLAGS.checkpoint_stereducer,
...
def main(argv):
    ...
    if rank < num_reducers:
        job_name = "reducer"
        task_index = rank
    else:
        job_name = "worker"
        task_index = rank - num_reducers
    ...
    with g.as_default():
        if job_name == "reducer" and task_index != 0:
            ...
            barrier = build_barrier(int(os.environ['SLURM_NTASKS']))
            config, _, server = create_cluster(job_name, task_index)
            for _ in range(FLAGS.num_tests):
                with tf.train.MonitoredTrainingSession(master=server.target,
                                    is_chief=False,
                                    chief_only_hooks=chief_only_hooks,
